...
public class MockPipeline extends Pipeline {
	...
	public Response<byte[]> echo(final byte[] string) {
		final Response<byte[]> response = new Response<>(BuilderFactory.BYTE_ARRAY);
		response.set(string);
		return response;
	}
	...
	public Response<Boolean> exists(final String key) {
		final Response<Boolean> response = new Response<>(BuilderFactory.BOOLEAN);
	...
	public Response<List<String>> mget(final String... keys) {
		...
		final List<byte[]> result = new ArrayList<>(keys.length);
	...
	public Response<Double> incrByFloat(final String key, final double increment) {
		final Response<Double> response = new Response<>(BuilderFactory.DOUBLE);
	...
	public Response<Map<String, String>> hgetAll(final String key) {
		final Response<Map<String, String>> response = new Response<>(BuilderFactory.STRING_MAP);
		final Map<DataContainer, DataContainer> result = mockStorage.hgetAll(DataContainer.from(key));

		if (result != null) {
			final List<byte[]> encodedResult = new ArrayList<>(result.size());
	...
	public Response<Map<byte[], byte[]>> hgetAll(final byte[] key) {
		final Response<Map<byte[], byte[]>> response = new Response<>(BuilderFactory.BYTE_ARRAY_MAP);
		final Map<DataContainer, DataContainer> result = mockStorage.hgetAll(DataContainer.from(key));
	...
	public Response<Set<String>> hkeys(final String key) {
		final Response<Set<String>> response = new Response<>(BuilderFactory.STRING_SET);
	...
	public Response<Set<byte[]>> hkeys(final byte[] key) {
		final Response<Set<byte[]>> response = new Response<>(BuilderFactory.BYTE_ARRAY_ZSET);
		final Set<DataContainer> result = mockStorage.hkeys(DataContainer.from(key));
		response.set(result == null ? new ArrayList<>(0) : DataContainer.toBytes(result));
		return response;
	}
	...
	public Response<List<String>> hmget(final String key, final String... fields) {
		final Response<List<String>> response = new Response<>(BuilderFactory.STRING_LIST);
		final List<byte[]> result = new ArrayList<>(fields.length);
